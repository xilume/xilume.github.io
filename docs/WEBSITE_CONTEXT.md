...
用户明确希望接手整个官网的长期维护，并逐步提升网站审美和设计；范围包含代码、产品页、双语内容、图片引用、导航、下载链接、技术 SEO、预览测试，以及获批后的发布验证。

保持现有技术栈、品牌风格和产品定位，不每次重新设计。首次仅交接和审查的阶段已经完成；随后用户要求“把你检查出来的问题和需要改进的地方都改好”，当前可在独立任务分支自主修复、测试并准备草稿审查，不逐行请求确认。这包括本轮维护工具、页面入口、语言导航和待审 workflow 差异；没有取消先前明确的 main 推送、PR 合并、正式部署与 Pages 设置批准边界，也不授权更换 Logo、改产品事实或域名。
...
每轮更新状态，将新的确认决定留在版本化文档。禁止把凭据、私人聊天或未公开商业信息写入公开仓库。尚未安排自动巡检或自动发布。

## 工程事实

仓库 `xilume/xilume.github.io` 为公开仓库，默认分支 main。当前远端基线与交接包相同，无需回退。技术栈为无应用构建步骤的静态 HTML/CSS/JavaScript；英文位于根目录，中文位于 `zh-cn/`。
...
2026-09-08 后续认证只读 API 已核实 Pages 设置为 `build_type: legacy`、来源分支 `main`、路径 `/`，域名 `xilume.co` 且强制 HTTPS；这补齐了首轮未读取设置的证据。没有修改任何 Pages 设置，也不能从两次成功记录确定哪次最终覆盖另一套。

当前任务分支准备以 `scripts/site_files.py` 作为唯一白名单来源，由 `scripts/build-site.py` 构建 `_site/` 并先检查；根资源显式列出，网站目录递归复制并排除维护资料。`.nojekyll` 保留且上传步骤显式包含隐藏文件。PR 运行工具、语言导航、artifact 与 JavaScript 检查，只有 `main` push 或在 `main` 手动运行才可能进入部署；PR 不上传 Pages artifact，不取消正在运行的正式部署。

**A1 尚未在线上解决。** 首次合并修复前必须取得明确的发布及设置切换授权，将 Pages Source 从分支发布切为 **GitHub Actions**（`build_type: workflow`），保留现有域名和 HTTPS，核实设置后再按授权合并及部署。仅修改 workflow 或白名单不能停止当前 legacy 动态流程。发布后需核对唯一预期入口、artifact 范围、关键页面和下载，并验证维护文件 URL 返回 404。即使以后不随站点发布，公开仓库中的维护文件仍不是秘密。
...
首轮浏览器超时已经在后续检查中解决，完成 1440px 桌面及 390px 手机宽度抽样、菜单/Escape、查询参数与锚点、前进/后退和视频暂停检查。具体证据与检查范围见 STATUS/AUDIT；这不等于全部浏览器、每张产品图、软件安装、硬件事实或搜索结果均已验证。未建立自动巡检、自动发布或后台持续运行。
...
同步命令仅用于获批的页面或共享资源维护，并审查 diff。其他命令检查与预览实际发布 artifact；预览地址为 `http://127.0.0.1:8000/`。系统使用 `python3` 时替换命令名。构建只可替换仓库自己的 `_site/`；自定义输出必须为仓库外新目录或空目录。以后新增根资源更新 `scripts/site_files.py`，新增可索引页面同步语言与 sitemap。正式发布仍须遵守前述授权和 Pages 设置前置条件。
...
- 2026-09-08 用户后续修复要求、认证只读 Pages API 核对、当前任务分支及 STATUS/AUDIT 中的验证记录
