...
> 历史报告：下文保留首轮只读审计的基线与当时结论，原行号指 `d188871`。同日后续修复状态见 `WEBSITE_STATUS.md`，发布方案见 `WEBSITE_RELEASE.md`。后续已修复 A2/A3/A4 及确认的语言切换问题 A5，并补充 A1 构建检查；A1 线上配置尚待获批切换。工作树现已完整检出，浏览器检查已恢复，Pages API 已确认 legacy/main/根目录；下文首次审计的“尚未”不是最新状态。
