...
阶段：PR #5 已合并并正式部署，线上文件验证通过；正式域名提交返回激活提示，正在确认官网来源激活
...
| 已线上验证 | 线上 201 文件均 200、116 文本资源与审查产物一致（CRLF 归一化）、6 类维护路径原 URL 和刷新 URL 均 404；正式页面显示新版表单。正式站提交尚待此次来源激活 |

用户取消私人邮箱转发关系核验，仅要求发往 `contact@xilume.co`；本轮遵循该决定。用户已确认点击首封表单激活，本地预览来源的实际中文提交得到服务接受，按钮恢复和内容清空，控制台无 error/warn。未读取实际收件邮件，不将此写成正式来源已接受、收件箱送达或私人转发已证实。服务资料、实现与限制见 `WEBSITE_CONTACT.md`。

线上文件核验时间为 2026-09-08 12:41:32 UTC，错误为零；公开浏览器提交返回待激活提示，已经请用户确认最新官网来源激活邮件。本地预览成功不代替正式来源激活，固定 `_url` 也不能当作实际来源已激活的证据。正式浏览器连接较慢，分步截图确认输入；没有因自动化读取邮箱值为空而认定网页丢失输入。
